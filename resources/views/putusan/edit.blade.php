        ...
        <div class="card-body">
            <form action="/update-putusan/{{ $produk->produk_id }}" method="post" enctype="multipart/form-data">
                ...
                <div class="mb-3">
                    ...
                    <textarea class="form-control" id="produk_judul" rows="2" name="produk_judul" placeholder="Judul Putusan"
                        value="{{ old('produk_judul', $produk->produk_judul) }}">{{ $produk->produk_judul }}</textarea>
                </div>

                <div class="row">
                    <div class="col mb-3">
                        <label class="form-label" for="produk_cluster"><strong>Klaster</strong></label>
                        <div class="col">
                            <select class="form-control" id="produk_cluster" name="produk_cluster">
                                @if ($produk->produk_cluster == null)
                                    <option selected hidden></option>
                                @endif

                                @foreach ($clusters as $item)
                                    @if (old('produk_cluster', $item->cluster_id) == $produk->produk_cluster)
