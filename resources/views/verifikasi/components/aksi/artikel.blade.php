@if ($item->verifikasiPeraturanLatest != null)
    @if ($item->verifikasiPeraturanLatest->aksi == 1 || $item->verifikasiPeraturanLatest->aksi == 3)
        <div class="d-flex justify-content-center">
            <a href="{{ route('artikel.detail', $item->produk_id) }}" class="btn btn-primary btn-circle mx-1"
    ...
    @else
        <div class="d-flex justify-content-center">
            <a href="{{ route('edit-artikel', $item->produk_id) }}" class="btn btn-info btn-circle mx-1"
                role="button">Edit</a>
            <form action="{{ route('delete-artikel', $item->produk_id) }}" method="POST" class="mx-1"
