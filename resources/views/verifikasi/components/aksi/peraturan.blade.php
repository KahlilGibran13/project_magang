@if ($item->verifikasiPeraturanLatest != null)
    @if ($item->verifikasiPeraturanLatest->aksi == 1 || $item->verifikasiPeraturanLatest->aksi == 3)
        <div class="d-flex justify-content-center">
            <a href="{{ route('peraturan.detail', $item->produk_id) }}" class="btn btn-primary btn-circle mx-1"
    ...
    @elseif($item->verifikasiPeraturanLatest->aksi == 5)
        <div class="d-flex justify-content-center">
            <div class="btn btn-success btn-circle mx-1">Tampil</div>
        </div>
    @else
        <div class="d-flex justify-content-center">
            <a href="/edit-peraturan/{{ $item->produk_id }}" class="btn btn-info btn-circle mx-1"
                role="button">Edit</a>
            <form action="{{ route('peraturan.destroy', $item->produk_id) }}" method="POST" class="mx-1"
...
@else
    <div class="d-flex justify-content-center">
        <a href="/edit-peraturan/{{ $item->produk_id }}" class="btn btn-info btn-circle mx-1" role="button">Edit</a>
        <form action="{{ route('peraturan.destroy', $item->produk_id) }}" method="POST" class="mx-1"
            onsubmit="return confirm('Apakah kamu yakin untuk menghapus ini?');">
            @csrf
            @method('DELETE')
