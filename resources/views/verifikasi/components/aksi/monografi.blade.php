@if ($item->verifikasiPeraturanLatest != null)
    @if ($item->verifikasiPeraturanLatest->aksi == 1 || $item->verifikasiPeraturanLatest->aksi == 3)
        <div class="d-flex justify-content-center">
            <a href="{{ route('monografi.detail', $item->produk_id) }}" class="btn btn-primary btn-circle mx-1"
    ...
    @else
        <div class="d-flex justify-content-center">
            <a href="{{ route('edit-monografi', $item->produk_id) }}" class="btn btn-info btn-circle mx-1"
                role="button">Edit</a>
            <form action="{{ route('delete-monografi', $item->produk_id) }}" method="POST" class="mx-1"
